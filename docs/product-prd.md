...
**描述**：核心交互入口。Typeless 语音优先体验——打开即录音，零摩擦输入。

**功能要点**：
- **语音输入（主）**：大圆形录音按钮，按住说话松开结束。调用腾讯云 ASR（一句话识别，5000 次/月免费额度）实时转写，结果可编辑修正
- **文字输入（辅）**：底部常驻文本区，作为备选入口
...
- 用户可点击追问快速回复，也可自由输入
- Agent 自主决定何时调用工具（searchReviews / getGoalProgress / getUserInsights 等）
- 对话≥2轮后异步提取洞察 → 写入 UserInsight
- AI 回复以教练口吻：客观、建设性、略带挑战
- 对话历史持久化
- 可选：连接 OpenClaw 本地实例，用 SOUL.md + MEMORY.md 做推理

**优先级**：P0 — MVP 必需（首轮追问 + Agent 架构）；P1（多轮对话 UI + OpenClaw）
...
| 手机号验证码登录 | 腾讯云 SMS（开发模式固定 111111） |
| 年度目标 CRUD | 设定 1-3 个年度目标 |
| Typeless 复盘输入 | 语音优先（按住说话），文字备选，无 Scope 选择器 |
| 语音识别 ASR | 腾讯云 ASR 一句话识别（5000次/月免费） |
| GDRR AI 结构化 | 提交文本 → DeepSeek 输出 GDRR + 标签 + 教练追问 |
| 复盘详情页 | GDRR 卡片展示 + 教练追问卡片 |
| 首页仪表盘 | 今日状态 + 目标进度 + 最近复盘 |
| 周期报告 | 周报/月报/季报，AI 生成 + 图表 + 年度目标进度 |
| 教练 Agent | Tool-use Agent 架构，function calling，记忆系统 |
